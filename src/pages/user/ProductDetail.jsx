    ...
    <div className="text-lg font-semibold flex  items-center">
      ...
      {selectedCoupon && (
        <div className="flex items-center">
          <span className=""><svg viewBox="0 0 20 20" className="w-6 h-6 rotate-120" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M2 9.5C2 9.22386 2.22386 9 2.5 9H17.5C17.7761 9 18 9.22386 18 9.5C18 9.77614 17.7761 10 17.5 10H2.5C2.22386 10 2 9.77614 2 9.5Z" fill="#212121"></path> </g></svg></span>
    ...
    </div>
    {
      applicableCoupons.length > 0 && (
        <button
          ...
          className="py-2 px-2 text-xs rounded-full border border-gray-800 cursor-pointer"
        >
          Apply Coupons
        </button>
      )
...
`}
  ...
  {selectedProduct.status === "Coming Soon" && (
    <div className="border-b border-gray-300 py-8">
     ...
     <div className="block sm:hidden">
      ...
      {mainImage && (
        mainImage.endsWith(".mp4") ? (
          <video
            src={mainImage}
            className="w-full h-[480px] cursor-pointer  object-cover"
            autoPlay
            loop
            muted
            playsInline
     ...
     <div  className=" mt-2 mb-0 overflow-x-auto flex w-full  gap-2">
  {/* phần tử con */}
  ...
  >
    <img
      src={selectedProduct?.img}
      alt={selectedProduct?.name}
      className="w-[100px] h-[100px] rounded-[4px] object-cover cursor-pointer"
    />
    <div className="absolute inset-0 bg-black/15 rounded-[4px] hover:bg-black/30 duration-300 ease-in-out transition" />
  </div>


   {/* các ảnh phụ cũng cần `flex-shrink-0` */}
    {selectedProduct?.additionalImages
      ?.filter((img) => typeof img === 'string' && img.trim() !== '')
      .map((img, idx) => {
        const isVideo = img.split('?')[0].toLowerCase().endsWith('.mp4');
          ...
          >
            {isVideo ? (
              <div className="relative">
                <video
                  src={img}
                  className="w-[100px] h-[100px] rounded-[4px] object-cover cursor-pointer hover:border-black transition"
                  muted
                  loop
                  playsInline
                  onError={(e) => {
...
<div className="py-4 px-6">
      ...
      )}
    {
      applicableCoupons.length > 0 && (
        <button
          onClick={() => setShowCoupons(!showCoupons)}
          className="py-2 px-2 text-sm rounded-full border border-gray-800 cursor-pointer"
        >
          Apply Coupons
        </button>
      )
