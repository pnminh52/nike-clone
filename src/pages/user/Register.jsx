...
const Register = () => {
  ...
  const addExtraAddress = () => {
    if (extraAddresses.length < 3) {
      setExtraAddresses([...extraAddresses, ""]);
    }
  };

  const handleExtraAddressChange = (index, value) => {
    const newExtraAddresses = [...extraAddresses];
    newExtraAddresses[index] = value;
    setExtraAddresses(newExtraAddresses);
  };
  const removeExtraAddress = (index) => {
    setExtraAddresses(prevAddresses => prevAddresses.filter((_, i) => i !== index));
  };
  
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!firstname || !lastname || !phone || !email || !password || address.length === 0) {
      warningToast("Vui lòng điền đầy đủ thông tin bắt buộc!");
      return;
    }
    if (!isValidLength || !isValidContent) {
      warningToast("Mật khẩu phải có ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường và số.");
      return;
    }

    if (extraAddresses.length > 0 && extraAddresses.every(addr => addr.trim() === "")) {
      errorToast("Vui lòng điền địa chỉ phụ");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="mt-7 mb-7 max-w-lg px-6 mx-auto">
      <div className="flex items-center gap-2 mb-4">
        <svg viewBox="0 0 24 24" fill="none" width="60px" height="60px">
          <path
            fill="currentColor"
  ...
  `}
          ...
          >
            ...
            {show ? (
              ...
              >
                ...
                <g
                  id="Icons-/-Forms-/-Password-/-Hide"
                  stroke="none"
                  stroke-width="1"
                  fill="none"
                  fill-rule="evenodd"
                >
                  <g id="Icon/Interface/Eye/Slash/Black">
                    <rect
                      id="Container"
                      x="0"
                      y="0"
    ...
    <p
        className={`flex gap-2 items-center ${
          isValidLength ? "text-green-600" : "text-[#757575]"
        }`}
      >
        {isValidLength ? (
          // ✅ Icon "Done"
          ...
          >
            <path
              d="M0.353516 3.79395L3.52218 6.96261L9.64618 0.83728"
              stroke="#007D48"
            ></path>
          </svg>
        ) : (
      ...
      <p
        className={`flex gap-2 items-center ${
          isValidContent ? "text-green-600" : "text-[#757575]"
        }`}
      >
        {isValidContent ? (
          // ✅ Icon "Done"
      ...
      <div className="text-right">
        <button
          type="submit"
          className="bg-black inter cursor-pointer rounded-full text-white px-7 py-3 mt-2 text-lg"
        >
          Create Account
        </button>
      </div>
